...
Unet_FPN_default=0
Fusion_manner=0 #0 is 1 now,
Trans_thick_aware=0
Sep_pre_Conv=0
# 0 qk concat adjacent slices then FC to fixed size,v use key slices;
# 1 qk concat all slices then FC to fixed size, v use the key slice;
# 2,qk Conv adjacent slices then FC to fixed size ,v use key slices;
# 3,qk Conv all slices then FC to fixed size ,v use key slices;


input_channel = dataset_transform['NUM_SLICES']
feature_channel = 512
if backbone_manner==0:
    backbone_dict=dict(
    ...
    backbone_dict = dict(
        ...
        fpn_finest_layer=2,
        memory_efficient=True)
elif backbone_manner==2:
    backbone_dict = dict(
        type='Patch_Trans_with_A3D',
        n_cts=input_channel,
        out_dim=feature_channel,
        fpn_finest_layer=2,
        memory_efficient=True)
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        # ann_file=data_root + '10train_ann.pkl',
        # ann_file=data_root + 'train_ann.pkl',
        ann_file=data_root + '5000train_ann.pkl',
...
log_level = 'INFO'
# work_dir = f'./work_dirs/5000densenet_3d_a3d_r2'
# work_dir = f'./work_dirs/densenet_3d_trans_a3d_r2'
work_dir = f'./work_dirs/5000densenet_3d_trans_a3d_r2'
load_from = None
# load_from =r''
resume_from = None
